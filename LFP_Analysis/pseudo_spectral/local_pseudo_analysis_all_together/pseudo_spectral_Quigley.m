...
load(strcat(dir_in_out_test,sprintf('test_stats_%s.mat',monkey)));
...
sess_range = [1,2,3];
Events = ["target","move"];
filename = sprintf('_pth_%.2f_iter_%d_nch_2',p_th,iterations);
ts = t_stats.ts;
ti = t_stats.ti;
...
pseudo_stats = null_distribution_subsampled(stats,Events,sess_range,f_max,iterations); 
...
save(strcat(dir_out_null,sprintf('pseudo_stats_%s%s.mat',monkey,filename)),'pseudo_stats','-v7.3');

disp(['Saving null stats averages and z-scored results ...'])
save(strcat(dir_out_null,sprintf('null_stats_%s%s.mat',monkey,filename)),'null_stats','-v7.3');
save(strcat(dir_out_test,sprintf('zscored_stats_%s_pth_%.2f%s.mat',monkey,p_th,filename)),'Zscored_stats','-v7.3');

...
% PSD and spectrogram for high/low optic flow density are computed by randomly swapping
% the lfp reward in high with those in low density optic flow.
%
...
% spectrogram difference between high and low optic flow - to be used to construct
% a Null Distribution, for each region and event
%
% @ Gino Del Ferraro, NYU, March 2023


function pseudo_stats = null_distribution_subsampled_ch_density(stats,Events,sess_range,f_max,permutations,ID_job)
...
for sess = sess_range
    for region = 1:length(reg_names)
        ...
        for EventType = Events
            ...
            nch = length(stats(1).region.(reg).event.(EventType).high_den.ch); % total number of channels, same for each session
            display(['-- brain region: ',num2str(reg),' - Event: ',num2str(EventType), ' - sess ',num2str(sess)])
            
            n1 = size(stats(sess).region.(reg).event.(EventType).high_den.ch(1).lfp,2); % number of incorrect trials (no reward)
            n2 = size(stats(sess).region.(reg).event.(EventType).low_den.ch(1).lfp,2); % number of correct trials (reward)
            ...
            for chnl = 1:nch
                ...
                X1 = stats(sess).region.(reg).event.(EventType).high_den.ch(chnl).lfp; % lfp matrix -> time x trial
                X2 = stats(sess).region.(reg).event.(EventType).low_den.ch(chnl).lfp; % lfp matrix -> time x trial
                ts_size = length(ts);

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This code perform the permutation test for the spectrogram difference
% (no-reward - reward) and for the theta/beta frequency vs time power.
% It is written to be run on the HPC server
% 
% Input: stats and test_stats structures
% Output: pseudo_stats distribution for a given number of permutations
%
% Gino Del Ferraro, Jan 2023, NYU


function [] = pseudo_spectral_server_density(monkey,N_iter,ID_job)
...
load(strcat(dir_in_test,sprintf('test_stats_%s_all_events_density.mat',monkey)));
...
pseudo_stats = null_distribution_subsampled_ch_density(stats,Events,sess_range,f_max,iterations,ID_job); 


disp(['Saving psuedo stats ...'])
save(strcat(dir_out_null,sprintf('pseudo_stats_%s_all_events_density.mat',filename_null)),'pseudo_stats','-v7.3');

...
for region = 1:length(reg_names)
    ...
    for EventType = Events
        ...
        % % THETA POWER
        fig = figure;
        shadedErrorBar(tsi,rwd1_theta_t,rwd1_err_theta,'lineprops',{'color',"#ff6600"},'patchSaturation',0.5); hold on % incorrect trials
        shadedErrorBar(tsi,rwd2_theta_t,rwd2_err_theta,'lineprops',{'color',"#00cc66"  },'patchSaturation',0.5); % correct trials
        ...
        % set(gca,'FontSize',14)
        grid on
        fname = strcat(dir_out_region,sprintf('%s_power_theta_vs_time_reg_%s_event_%s.png',monkey,reg,EventType));
        saveas(fig,fname)
        
        
        % % BETA POWER
        fig = figure;
        shadedErrorBar(tsi,rwd1_beta_t,rwd1_err_beta,'lineprops',{'color',"#ff6600"},'patchSaturation',0.5); hold on % incorrect trials
        shadedErrorBar(tsi,rwd2_beta_t,rwd2_err_beta,'lineprops',{'color',"#00cc66"  },'patchSaturation',0.5); % correct trials
        ...
        % set(gca,'FontSize',14)
        grid on
        fname = strcat(dir_out_region,sprintf('%s_power_beta_vs_time_reg_%s_event_%s.png',monkey,reg,EventType));
        saveas(fig,fname)

...
s = experiments.sessions(1).lfps(1).stats.trialtype.reward(1).events.target.all_freq;
...
% Find NaN across: sessions, events, rewards, channels, and trials
[ts_start, ts_stop] = NaN_ts_start_monkey_density(experiments,Events,sess_range,rwd_range) % find first time index with no NaN 
ts_stop = min(ts_stop,1.8); % end of time series to be analyzed 
nch = size(experiments.sessions(sess).lfps,2); % # of total channels
...
NaN_plots_all_trials(experiments,"stop",3)

...
function [ts_start_tot, ts_stop_tot] = NaN_ts_start_monkey_density(experiments,Events,sess_range)
...
for sess = sess_range
    for EventType = Events
            [start, stop] = NaN_idx_density(experiments,sess,EventType); % find first time index with no NaN across all channels and trials
            ts = experiments.sessions(sess).lfps(1).stats.trialtype.all.events.(EventType).all_freq.ts_lfp_align_ext; % lfp structure

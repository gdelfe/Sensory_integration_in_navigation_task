...
function [coh_matrix] = get_triangular_matrix_values_of_coherence_rwd(coh,r,reg_ni)

F = size(coh.rwd(r).reg_X.(reg_ni).reg_Y.(reg_ni).mat,3); % frequency length
N =  size(coh.rwd(r).reg_X.(reg_ni).reg_Y.(reg_ni).mat,1); % matrix size (for region i)
M = N*(N-1)/2; % number of elements in the upper triangular part of the matrix

coh_matrix = zeros(M,F);
for f = 1:F
    coh_matrix(:,f) = nonzeros(coh.rwd(r).reg_X.(reg_ni).reg_Y.(reg_ni).mat(:,:,f))';
end

    ...
    for region_i = 1:length(reg_names)
        ...
        for region_j = region_i:length(reg_names)
            ...
            for r = 1:2
                ...
                %             % phase vs time
                %             phase_theta = coh_avg_rwd.(den).reg_X.(reg_i).reg_Y.(reg_j).angle(:,theta_idx)'; % coherencegram theta band
                %             phase_beta = coh_avg_rwd.(den).reg_X.(reg_i).reg_Y.(reg_j).angle(:,beta_idx)'; % coherencegram beta band
                %

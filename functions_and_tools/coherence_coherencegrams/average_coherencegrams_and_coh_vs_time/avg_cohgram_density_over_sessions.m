...
%                     angle_tot_std = cat(3,angle_tot_std, angle_std.^2 + angle.^2); % to compute the std coherencegram across session
                ...
                % std across sessions
                coh_avg.(den).(EventType).reg_X.(reg_i).reg_Y.(reg_j).cohgram_std = sqrt(mean(coh_tot_std,3) - mean(coh_tot,3).^2);
                coh_avg.(den).(EventType).reg_X.(reg_i).reg_Y.(reg_j).cohgram_sem = sqrt(mean(coh_tot_std,3) - mean(coh_tot,3).^2)/sqrt(rand_ch*(rand_ch-1)/2*3);
